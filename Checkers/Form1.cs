...
namespace Checkers {
    ...
    public static class World {
        ...
        public static void Init() {
            checkers = new Checker[16] {
                new Checker(1, 0, 1), new Checker(3, 0, 1),
                new Checker(5, 0, 1), new Checker(7, 0, 1), 
                new Checker(0, 1, 1), new Checker(2, 1, 1),
                new Checker(4, 1, 1), new Checker(6, 1, 1),
                // white

                new Checker(0, 7, 2), new Checker(2, 7, 2),
                new Checker(4, 7, 2), new Checker(6, 7, 2),
                new Checker(1, 6, 2), new Checker(3, 6, 2),
                new Checker(5, 6, 2), new Checker(7, 6, 2),
                // black
            };

            screen = new Bitmap(1000, 1000);
            screenG = Graphics.FromImage(screen);
            ReDraw(); // create checkers
        } 
        ...
        public static void Click(float positionX, float positionY, bool isLeft) {
            ...
            if (isLeft) { foreach (var c in checkers) { c.TrySelect(placeX, placeY); } }
            else { foreach (var c in checkers) { c.TryMove(placeX, placeY); } }
        ...
        public static void ReDraw() {
            ...
            foreach (Checker c in checkers) { c.SelfDraw(); }
        } // перерисовка
    ...
    public class Checker {
        ...
        public void TrySelect(int clickPlaceX, int clickPlaceY) {
            ...
            if (placeX == clickPlaceX && placeY == clickPlaceY) { isSelect = true; }
        ...
        public void Destroy() { World.checkers = World.checkers.Where(x => x != this).ToArray(); }
    } 
